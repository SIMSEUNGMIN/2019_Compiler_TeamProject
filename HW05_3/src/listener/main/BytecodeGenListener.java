...
public class BytecodeGenListener extends MiniCBaseListener implements ParseTreeListener {
    ...
    public void exitVar_decl(MiniCParser.Var_declContext ctx) {
        ...
        if (isDeclWithInit(ctx)) {
            ...
            if (Float.parseFloat(varValue) >= 6) {
                if (isIntDecl(ctx)) {
                    varDecl += thisString +
                            "bipush " + varValue + "\n" +
    ...
    public void exitLocal_decl(MiniCParser.Local_declContext ctx) {
        String varDecl = "";
        if (isDeclWithInit(ctx)) {//값 초기화
            if (isIntDecl(ctx)) {//int
                symbolTable.putLocalVarWithInitVal(getLocalVarName(ctx), INT, initVal(ctx));
                String vId = symbolTable.getVarId(ctx);
                varDecl += "ldc " + ctx.LITERAL().getText() + "\n"
                        + "istore_" + vId + "\n";
            } else if (isFloatDecl(ctx)) {//float
                symbolTable.putLocalVarWithInitVal(getLocalVarName(ctx), FLOAT, initVal(ctx));
                String vId = symbolTable.getVarId(ctx);
                varDecl += "ldc " + ctx.LITERAL().getText() + "\n"
                        + "fstore_" + vId + "\n";
        ...
        } else if (isArrayDecl(ctx) && isIntDecl(ctx)) {//int[]
            String vId = symbolTable.getVarId(ctx);
            String reset = "";
            ...
            varDecl += "iconst_" + ctx.getChild(3) + "\n"
                    + "newarray\tint\n"
                    + reset + "astore_" + vId + "\n";
        } else if (isArrayDecl(ctx) && isFloatDecl(ctx)) {//float[]
            String vId = symbolTable.getVarId(ctx);
            String reset = "";
            for (int i = 0; i < Integer.parseInt(ctx.getChild(3).getText()); i++) {
                reset += ("dup\n"
    ...
    public void exitExpr(MiniCParser.ExprContext ctx) {
        ...
        if (ctx.getChildCount() == 1) { // IDENT | LITERAL
            if (ctx.IDENT() != null) {
                ...
                boolean isLocal = symbolTable.isLocal(idName);
                Type varType = symbolTable.getVarType(idName);
                String varId = symbolTable.getVarId(idName);
                if (!isLocal && (varType == INT)) { //IDENT가 전역 변수일 경우
                    expr += "getfield " + "Test/" + idName + " " + "I" + "\n";
                } else if (!isLocal && (varType == FLOAT)) { //IDENT가 전역 변수일 경우
                    expr += "getfield " + "Test/" + idName + " " + "F" + "\n";
                } else if (varType == INT) {
                    expr += "iload_" + varId + " \n";
                } else if (varType == FLOAT) {
                    expr += "fload_" + varId + "\n";
                } else if (varType == Type.INTARRAY || varType == Type.FLOATARRAY) {
                    expr += "aload_" + varId + "\n";
                }
                //else	// Type int array => Later! skip now..
                //	expr += "           lda " + symbolTable.get(ctx.IDENT().getText()).value + " \n";
            } else if (ctx.LITERAL() != null) {//just number
            ...
            if (ctx.getChild(0).getText().equals("(")) {        // '(' expr ')'
                ...
                Type isVarType = symbolTable.getVarType(idName);
                //만약 전역변수라면
                if (isVar) {
                    idName = ctx.IDENT().getText();
                    if (isVarType == INT) {
        ...
        // IDENT '[' expr ']' '=' expr
            ...
            if (ctx.getChild(4).getText().equals("=")) {
                ...
                expr1 = "iconst_" + expr1;
                Type isVarType = symbolTable.getVarType(ctx.IDENT().getText());
                if (isVarType == FLOAT)
                    expr2 = "fconst_" + expr2 + "\n" + "fastore\n";
                else
                    expr2 = "iconst_" + expr2 + "\n" + "iastore\n";
